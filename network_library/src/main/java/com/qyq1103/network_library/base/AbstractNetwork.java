...
public abstract class AbstractNetwork implements IEnvironment {
  ...
  protected <T> T initRxCache(Class service) {
    ...
    rxCaHashMap.put(
        service.getName() + baseUrl,
        cacheBuilder.persistence(cacheDir, new GsonSpeaker()).using(service));
    ...
    if (okHttpClient == null) {
      ...
      if (getInterceptors() != null && !getInterceptors().isEmpty()) {
        for (int i = 0; i < getInterceptors().size(); i++) {
          httpClientBuilder.addInterceptor(getInterceptors().get(i));
        }
      }
